# -*- coding: utf-8 -*-
"""
States to manage vcsh git repositories and configuration
...
def __virtual__():
    """
    Only load if vcsh is available
    """
    if "vcsh.version" not in __salt__:
        return (False, "vcsh module could not be loaded")
    if "git.version" not in __salt__:
        return (False, "vcsh module could not be loaded: git module not available")
    ...
    **kwargs
    ...
    try:
        desired_fetch_url = salt.utils.url.add_http_basic_auth(
            target, https_user, https_pass, https_only=True
        )
    ...
    log.info("Checking remote revision for %s", target)
    try:
    ...
    if os.path.isdir(gitdir):
        ...
        log.debug("Target %s is not found, 'vcsh clone' is required", name)
        if __opts__["test"]:
            ret["changes"]["new"] = name + " => " + target
            return _neutral_test(
                ret, "Vcsh reporitoy {0} would be cloned from {1}".format(name, target)
            )
        try:
            clone_opts = []
            if remote != "origin":
...
def present(
    ...
    """
    Ensure that a vcsh repository exists in the given directory
    ...
    # If name is defined in vcsh return True
