...
def __virtual__():
    ...
    """
    if salt.utils.path.which("vcsh") is None:
        return (False, "The vcsh execution module cannot be loaded: vcsh unavailable.")
    return True
...
def add(
    ...
    repo
        The repo name to the git checkout
    ...
    """
    command = ["vcsh", repo, ] + _format_git_opts(git_opts)
...
def branch(
    ...
    repo
        The vcsh repo to the git checkout
...
def clone(
    ...
    .. code-block:: bash
        ...
        salt myminion vcsh.clone mydotfile git://github.com/saltstack/salt.git
    """
    if not url:
